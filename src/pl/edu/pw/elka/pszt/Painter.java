...
public class Painter extends JFrame {
	...
	private void paintEntity(final ArrayList<Segment> segments,
			final ArrayList<Segment> multiSegments, final int moveX,
				...
				if (CONTOURS) {
					for (Segment segment : multiSegments) {
						...
						for (int i = 0; i < multiVertices.size(); ++i) {
							g2.drawLine(
									50 + correctionX + (int) scaleX
											* (int) multiVertices.get(i).getX(),
									50 + correctionY + (int) scaleY
											* (int) multiVertices.get(i).getY(),
									50
											+ correctionX
											+ (int) scaleX
											* (int) multiVertices.get(
													(i + 1)
															% multiVertices
																	.size())
													.getX(),
