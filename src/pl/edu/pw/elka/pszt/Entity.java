...
public class Entity implements Comparable<Entity> {
	...
	public Entity copulateWith(final Entity other, int copulateGeneration) {
		...
		for (int i = 0; i < connectors.size(); ++i) {
			...
			if (connectors.get(i).getSegment().getId() != other.connectors
					.get(i).getSegment().getId()) {
			...
			if ((connectors.get(i).getSegmentVertex().getId() != other.connectors
					.get(j).getSegmentVertex().getId())
					|| (connectors.get(i).getTargetSegmentVertex().getId() != other.connectors
							.get(j).getTargetSegmentVertex().getId())) {
				++numberDifferent;
			}
		}
		...
		// numberTakenFromThis = (int) ((double) numberDifferent * 0.5 * (min /
		// max));
		// numberTakenFromOther = numberDifferent - numberTakenFromThis;
		//
		// } else {
		// numberTakenFromOther = (int) ((double) numberDifferent * 0.5 * (min /
		// max));
	...
	public Entity copulateWith2(final Entity other, int copulateGeneration) {
		...
		for (int i = 0; i < this.connectors.size(); ++i) {
			...
			int newVertexID = con1.getSegmentVertex().getId()
					+ con2.getSegmentVertex().getId();
			newVertexID = (int) (newVertexID * 0.5);

			int newTargetID = con1.getTargetSegmentVertex().getId()
					+ con2.getTargetSegmentVertex().getId();
	...
	public void mutateEntity() {
		...
		while (mutationStrenght > 0) {
			...
			if (generator.nextBoolean()) {
				...
				// just rotation
				targetSegmentVertex = connectorToMutate
						.getTargetSegmentVertex();
			}
	...
	public int compareTo(Entity other) {
		return (this.getAdaptationSize() < other.getAdaptationSize() ? -1
				: (this.getAdaptationSize() > other.getAdaptationSize() ? 1
