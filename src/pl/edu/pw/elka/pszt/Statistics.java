	...
	private class MyPanel extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			for (int i = 0; i < adapt.size() - 1; ++i) {
				g2.drawLine((int) adapt.get(i).x, (int) (dim.getHeight()
						- adapt.get(i).y - 200), (int) adapt.get(i + 1).x,
						(int) (dim.getHeight() - adapt.get(i + 1).y - 200));
				circle = new Ellipse2D.Float((int) adapt.get(i).x - 3,
						(int) (dim.getHeight() - adapt.get(i).y - 200) - 3,
						6.0f, 6.0f);
				g2.draw(circle);
			}
			circle = new Ellipse2D.Float(
					(int) adapt.get(adapt.size() - 1).x - 3,
					(int) (dim.getHeight() - adapt.get(adapt.size() - 1).y - 200) - 3,
					6.0f, 6.0f);
			g2.draw(circle);
			float dash[] = { 10.0f };
			...
			g2.drawLine(
					0,
					(int) (dim.getHeight() - adapt.get(adapt.size() - 1).y - 200),
					(int) dim.getWidth(),
					(int) (dim.getHeight() - adapt.get(adapt.size() - 1).y - 200));

			g2.drawLine(
					0,
					(int) (dim.getHeight() - (dim.getHeight() / 2) * 0.5 - 200),
					(int) dim.getWidth(),
					(int) (dim.getHeight() - (dim.getHeight() / 2) * 0.5) - 200);
		}
	}
