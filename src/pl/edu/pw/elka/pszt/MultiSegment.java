...
/**
 * Class containing representation of app model such as segments and target
 * segment made of small tangram segments
 * 
 * @author Marcin Kubik
...
public class MultiSegment {
	...
	/**
	 * Calculates area of multipolygon (minus intersection things - only cover
	 * area)
	 */
	private void calculateArea() {
		...
		for (int i = 0; i < this.segments.size(); ++i) {
			...
			for (int j = i + 1; j < this.segments.size(); ++j) {
				Geometry g = this.segments.get(i).getPolygon()
						.intersection(this.segments.get(j).getPolygon());
	...
	/**
	 * Align vertices to top left corner by removing minimum of X and Y
	 * coordinate
	 * 
	 ...
	 * 
	 * @param other
	 *            Multi segment to check with
	 * @return intersection area
	 * @return array of sum of intersection source with target, source
	 *         overlapping area, target covering area
	 */
	public double[] calculateIntersectionArea(final MultiSegment other) {
		...
		for (int i = 0; i < sourceSegments.size(); ++i)
			for (int j = 0; j < targetSegments.size(); ++j) {
				Geometry g = sourceSegments.get(i).getPolygon()
						.intersection(targetSegments.get(j).getPolygon());

				sumOfIntersection += g.getArea();
			}

		return new double[] { sumOfIntersection, source.getOverlapArea(),
				target.getCoveredArea() };
	}
